package com.leidos.lanemerge.consumers.gps;
...
public class GpsScheduledMessageConsumer implements Runnable {
    ...
    public GpsScheduledMessageConsumer()   {
        this.serverIp = LaneMergeApplicationContext.getInstance().getAppConfig().getGpsHost();
        this.serverTcpPort = LaneMergeApplicationContext.getInstance().getAppConfig().getGpsPort();
        this.clientUdpPort = LaneMergeApplicationContext.getInstance().getAppConfig().getGpsUdpPort();

        this.mabUdpPort = LaneMergeApplicationContext.getInstance().getAppConfig().getIntValue("mab.gps.port");
    ...
    public void run() {
        ...
        try{
            ...
            while (!bShutdown.get())  {
                ...
                //      but only if there is another writeToDisk somewhere else in a running thread (if this consumer
                //      is turned off we still want logs to be written!)
                try   {
    ...
    public void sendScheduleMessages() throws Exception   {
        ...
        int iFrequency = LaneMergeApplicationContext.getInstance().getAppConfig().getIntValue("gps.frequency");
